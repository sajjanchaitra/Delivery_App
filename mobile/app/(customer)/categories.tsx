...
export default function CategoriesScreen() {
  ...
  useEffect(() => { setPage(1); setProducts([]); setHasMore(true); fetchProducts(1, true); }, [selectedCategory, sortBy]);
  ...
  const fetchCategories = async () => {
    try {
      ...
      if (data.success && data.categories) {
        const allCategories: Category[] = [{ id: "all", name: "All", itemCount: 0 }, ...data.categories];
        allCategories[0].itemCount = data.categories.reduce((acc: number, cat: Category) => acc + (cat.itemCount || 0), 0);
  ...
  const fetchProducts = async (pageNum: number = 1, reset: boolean = false) => {
    try {
      ...
      const headers: Record<string, string> = { "Content-Type": "application/json" };
      if (token) headers["Authorization"] = `Bearer ${token}`;
      const queryParams = new URLSearchParams({ page: String(pageNum), limit: "20", sort: sortBy === "newest" ? "createdAt" : sortBy });
      if (selectedCategory && selectedCategory !== "All") queryParams.append("category", selectedCategory);
      const response = await fetch(`${API_URL}/api/customer/products?${queryParams.toString()}`, { headers });
      const data = await response.json();
      if (data.success && data.products) {
        if (reset || pageNum === 1) setProducts(data.products); else setProducts((prev) => [...prev, ...data.products]);
    ...
    } catch (error) { console.error("Error fetching products:", error); Alert.alert("Error", "Failed to load products"); }
    finally { setLoading(false); setRefreshing(false); setLoadingMore(false); }
  };

  const onRefresh = () => { setRefreshing(true); fetchCategories(); setPage(1); setHasMore(true); fetchProducts(1, true); };
  const loadMore = () => { if (!loadingMore && hasMore) fetchProducts(page + 1); };

  const addToCart = async (product: Product) => {
    try {
      const token = await AsyncStorage.getItem("authToken");
      if (!token) { Alert.alert("Login Required", "Please login to add items to cart", [{ text: "Cancel", style: "cancel" }, { text: "Login", onPress: () => router.push("/(auth)/login" as any) }]); return; }
      const response = await fetch(`${API_URL}/api/cart/add`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ productId: product._id, quantity: 1 }) });
      const data = await response.json();
      if (data.success) Alert.alert("Success", `${product.name} added to cart!`); else Alert.alert("Error", data.message || data.error || "Failed to add to cart");
    } catch (error) { console.error("Error adding to cart:", error); Alert.alert("Error", "Failed to add to cart"); }
  };

  const getSortLabel = (sort: SortOption): string => {
    switch (sort) { case "newest": return "Newest"; case "price-low": return "Price: Low to High"; case "price-high": return "Price: High to Low"; case "popular": return "Most Popular"; default: return "Newest"; }
  };
  ...
  const renderProductCard = ({ item: product }: { item: Product }) => {
    ...
    return (
      <TouchableOpacity style={styles.productCard} activeOpacity={0.8} onPress={() => router.push({ pathname: "/(customer)/product-details", params: { productId: product._id } } as any)}>
        ...
        <View style={styles.productInfo}>
          ...
          <View style={styles.productFooter}>
            <View style={styles.priceContainer}>
              <Text style={styles.productPrice}>{`₹${String(finalPrice)}`}</Text>
              {hasDiscount ? (
                <Text style={styles.originalPrice}>{`₹${String(product.price)}`}</Text>
              ) : null}
            </View>
            {product.inStock !== false ? (
              <TouchableOpacity style={styles.addButton} activeOpacity={0.7} onPress={(e) => { e.stopPropagation(); addToCart(product); }}>
  ...
  const renderFooter = () => {
    ...
    return <View style={styles.loadingMore}><ActivityIndicator size="small" color="#22C55E" /></View>;
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.categorySection}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoriesContainer}>
          {categories.map((category, index) => (
            <TouchableOpacity
              key={category.id || category._id || `cat-${String(index)}`}
            ...
            >
              ...
              {category.itemCount !== undefined && category.itemCount > 0 ? (
                <Text style={[styles.categoryCount, selectedCategory === category.name && styles.categoryCountActive]}>
      ...
      <View style={styles.filterBar}>
        <Text style={styles.resultsText}>{`${String(totalProducts)} ${totalProducts === 1 ? "Product" : "Products"}`}</Text>
        ...
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowSortModal(false)}>
          <View style={styles.sortModal}>
            ...
            {(["newest", "price-low", "price-high", "popular"] as SortOption[]).map((option) => (
              <TouchableOpacity key={option} style={styles.sortOption} onPress={() => { setSortBy(option); setShowSortModal(false); }}>
                <Text style={[styles.sortOptionText, sortBy === option && styles.sortOptionTextActive]}>{getSortLabel(option)}</Text>
                {sortBy === option ? <Ionicons name="checkmark" size={20} color="#22C55E" /> : null}
              </TouchableOpacity>
            ))}
          </View>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  categorySection: { backgroundColor: "#FFFFFF", paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: "#F1F5F9" },
  categoriesContainer: { paddingHorizontal: 16, gap: 8 },
  categoryChip: { flexDirection: "row", alignItems: "center", paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, backgroundColor: "#F1F5F9", marginRight: 8 },
  ...
  categoryCountActive: { color: "#DCFCE7" },
  filterBar: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: 16, paddingVertical: 12, backgroundColor: "#FFFFFF", borderBottomWidth: 1, borderBottomColor: "#F1F5F9" },
  ...
  browseButton: { marginTop: 20, backgroundColor: "#22C55E", paddingHorizontal: 24, paddingVertical: 12, borderRadius: 8 },
  ...
  storeName: { fontSize: 12, color: "#64748B", marginBottom: 4 },
